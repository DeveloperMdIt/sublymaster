...
const dbPath = path.join(__dirname, 'sublimaster.db');
...
function initDb() {
    db.serialize(() => {
        ...
        db.run(`ALTER TABLE projects ADD COLUMN offset_top REAL DEFAULT 0`, () => {});
        db.run(`ALTER TABLE projects ADD COLUMN offset_left REAL DEFAULT 0`, () => {});
        ...
        db.run(`CREATE TABLE IF NOT EXISTS print_templates (
            ...
            if (!err) {
                 ...
                 db.get("SELECT count(*) as count FROM print_templates", (err, row) => {
                    if (row && row.count === 0) {
                        db.run("INSERT INTO print_templates (name, width_mm, height_mm, orientation) VALUES ('Standard Tasse', 200, 95, 'portrait')");
                        db.run("INSERT INTO print_templates (name, width_mm, height_mm, orientation) VALUES ('Bierkrug', 230, 150, 'portrait')");
                        db.run("INSERT INTO print_templates (name, width_mm, height_mm, orientation) VALUES ('Mousepad', 230, 190, 'landscape')");
            ...
            is_active INTEGER DEFAULT 1
             ...
             if (!err) {
                 ...
                 db.get("SELECT count(*) as count FROM plans", (err, row) => {
                    if (row && row.count === 0) {
                        db.run("INSERT INTO plans (name, price, type, credits) VALUES ('Free', 0, 'subscription', 0)");
                        db.run("INSERT INTO plans (name, price, type, credits) VALUES ('Pro Monthly', 9.99, 'subscription', 0)");
                        db.run("INSERT INTO plans (name, price, type, credits) VALUES ('10er Karte', 15.00, 'credits', 10)");
        ...
        db.run("ALTER TABLE users ADD COLUMN offset_top REAL DEFAULT 0", () => {});
        db.run("ALTER TABLE users ADD COLUMN offset_left REAL DEFAULT 0", () => {});
        db.run("ALTER TABLE users ADD COLUMN credits INTEGER DEFAULT 0", () => {});

        // New Columns for Printer Intelligence
        db.run("ALTER TABLE users ADD COLUMN printer_model TEXT", () => {});
        db.run("ALTER TABLE users ADD COLUMN default_offset REAL DEFAULT 0", () => {});
        ...
        db.run(`CREATE TABLE IF NOT EXISTS printer_models (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        db.run(`CREATE TABLE IF NOT EXISTS print_feedback (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER,
            printer_model TEXT,
            used_offset REAL,
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        db.get('SELECT id FROM users WHERE email = ?', [email], async (err, row) => {
            ...
            db.run('INSERT INTO users (email, password, role, plan_id, credits) VALUES (?, ?, ?, ?, ?)', 
                ...
                // Log Activity
                db.run('INSERT INTO user_activity (user_id, action, ip_address) VALUES (?, ?, ?)', [userId, 'REGISTER', ip]);
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        db.get('SELECT * FROM users WHERE email = ?', [email], async (err, user) => {
            ...
            // Log Activity
            db.run('INSERT INTO user_activity (user_id, action, ip_address) VALUES (?, ?, ?)', [user.id, 'LOGIN', ip]);
...
app.get('/api/templates', authenticateToken, (req, res) => {
    db.all('SELECT * FROM templates WHERE user_id = ? ORDER BY created_at DESC', [req.user.id], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
});
...
app.post('/api/templates', authenticateToken, (req, res) => {
    ...
    db.run('INSERT INTO templates (user_id, name, width, height, data) VALUES (?, ?, ?, ?, ?)', 
...
app.get('/api/projects', authenticateToken, (req, res) => {
    ...
    db.all('SELECT id, name, thumbnail, template, offset_top, offset_left, created_at, updated_at FROM projects WHERE user_id = ? ORDER BY updated_at DESC', [req.user.id], (err, rows) => {
        if (err) {
...
app.get('/api/projects/:id', authenticateToken, (req, res) => {
    db.get('SELECT * FROM projects WHERE id = ? AND user_id = ?', [req.params.id, req.user.id], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/projects', authenticateToken, (req, res) => {
    ...
    db.get('SELECT id FROM projects WHERE user_id = ? AND name = ?', [req.user.id, name], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });

        if (row) {
            // Update existing project
            db.run('UPDATE projects SET data = ?, thumbnail = ?, template = ?, offset_top = ?, offset_left = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
            ...
            // Create new project
            db.run('INSERT INTO projects (user_id, name, data, thumbnail, template, offset_top, offset_left) VALUES (?, ?, ?, ?, ?, ?, ?)', 
...
app.get('/api/admin/users', authenticateAdmin, (req, res) => {
    db.all(`SELECT id, email, role, plan_id, created_at FROM users`, [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
});
...
app.put('/api/admin/users/:id', authenticateAdmin, (req, res) => {
    ...
    db.run('UPDATE users SET role = COALESCE(?, role), plan_id = COALESCE(?, plan_id), account_status = COALESCE(?, account_status) WHERE id = ?', 
...
app.delete('/api/admin/users/:id', authenticateAdmin, (req, res) => {
    ...
    db.serialize(() => {
        ...
        db.run('DELETE FROM user_activity WHERE user_id = ?', [id]);
...
app.get('/api/admin/stats', authenticateAdmin, (req, res) => {
    ...
    db.get("SELECT COUNT(*) as total FROM users", (err, row) => {
        ...
        db.get("SELECT COUNT(*) as pro FROM users WHERE plan_id > 1 AND role = 'user'", (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
            stats.proUsers = row ? row.pro : 0;

            db.all("SELECT id, user_id, action, ip_address, timestamp FROM user_activity ORDER BY timestamp DESC LIMIT 10", (err, rows) => {
                 if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/admin/settings', authenticateAdmin, (req, res) => {
    ...
    app.get('/api/settings', authenticateToken, (req, res) => {
        db.get('SELECT offset_top, offset_left FROM users WHERE id = ?', [req.user.id], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
    ...
    app.put('/api/settings', authenticateToken, (req, res) => {
        ...
        db.run('UPDATE users SET offset_top = ?, offset_left = ? WHERE id = ?', 
    ...
    app.post('/api/admin/plans', authenticateAdmin, (req, res) => {
        ...
        db.run('INSERT INTO plans (name, price, type, credits) VALUES (?, ?, ?, ?)', 
    ...
    app.put('/api/admin/plans/:id', authenticateAdmin, (req, res) => {
        ...
        const { id } = req.params;
        db.run('UPDATE plans SET name = ?, price = ?, type = ?, credits = ?, is_active = ? WHERE id = ?', 
    ...
    app.post('/api/checkout', authenticateToken, async (req, res) => {
        ...
        db.get("SELECT value FROM settings WHERE key = 'stripe_secret_key'", async (err, row) => {
            ...
            if (event.type === 'checkout.session.completed') {
                ...
                if (planId) {
                     if (planId === '10_credits_pack' || planId === 'credits_10') {
                        db.run('UPDATE users SET credits = credits + 10, account_status = "active" WHERE id = ?', [userId]);
                     } else if (planId === 'pro_monthly') {
                         db.run('UPDATE users SET plan_id = 2, account_status = "active" WHERE id = ?', [userId]);
                     }
                }
                ...
                // db.run('UPDATE users SET account_status = "suspended" WHERE stripe_customer_id = ?', [customerId]);
            }
    ...
    app.get('/api/printer/stats', (req, res) => {
        ...
        db.get('SELECT average_offset, use_count FROM printer_models WHERE name = ?', [model], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
    ...
    app.post('/api/printer/feedback', authenticateToken, (req, res) => {
        ...
        // 1. Log Feedback
        db.run('INSERT INTO print_feedback (user_id, printer_model, used_offset, status, correction_value) VALUES (?, ?, ?, ?, ?)',
        ...
        if (status === 'success') {
             ...
             db.get('SELECT * FROM printer_models WHERE name = ?', [printerModel], (err, row) => {
                 if (!row) {
                     // Create new
                     db.run('INSERT INTO printer_models (name, average_offset, use_count) VALUES (?, ?, 1)', [printerModel, usedOffset]);
                 } else {
                     // Update
                     const newCount = row.use_count + 1;
                     const newAvg = ((row.average_offset * row.use_count) + usedOffset) / newCount;
                     db.run('UPDATE printer_models SET average_offset = ?, use_count = ? WHERE id = ?', [newAvg, newCount, row.id]);
                 }
             });
        }
